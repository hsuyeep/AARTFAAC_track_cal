...
As the calibration algorithm's major, minor cycle and the WSF (Nelder-Mead
Simplex) are iterative in nature, we chose a cpu based approach using C++ as
the main language in combination with Eigen3 \citep{eigenweb}. Eigen 3 is A
high performance template library for linear algebra with a clean and
expressive API. \\
...
flags bad antennae and calibrates each individual channel per thread. Finally a
single image is created by combining the channels again, see Figure
\ref{fig:pipeline}.
