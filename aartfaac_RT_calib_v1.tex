...
\hline 
Array elements & 288 inverted V antennas & 128 tiles  & 256 wiregrid bowtie & Dual polarized elements\\ %\tabularnewline
% \hline 
...
\tablefoot{The parameters are estimated at the highest sensitivity
...
\ref{fig:The-AARTFAAC-calibration}  shows  the top  level  view  of the  AARTFAAC
...
\begin{tabular}{c c c c c}
\hline \hline
Src & $S_{60}(Jy)$ & 74 MHz Size  & \% Flux contribution  & Comment \\ % \tabularnewline
\hline 
% \hline 
3C461 (CasA) & 19605.31 & 7' & $\sim$50 & Baars spectra, helmboldt decay\\ % \tabularnewline
% \hline 
3C405 (CygA) & 19201.65 & 4.5' & $\sim$42 & Baars spectra\\ %\tabularnewline
% \hline 
3C144 (TauA) & 2417.31 & 8' & $\sim$11 & Baars spectra\\ % \tabularnewline
% \hline 
3C274 (VirA) & 1656.46 & 14' & $\sim$8& Helmboldt spectra\\ %\tabularnewline
\hline 
\end{tabular}}
...
Source & $S_{60}$ (Jy) & $\sigma_{S_{60}}$ & Calibrated & error & Ratio & Comment\\ %\tabularnewline
 &  &  & flux (Jy) & (Jy) &  & (vals derived from Obs. 3\\ %\tabularnewline
\hline 
3C10 & 367.9 & 18.81 & 397.02 &  & 1.08 & VLSS 4 comp.\\ % \tabularnewline
% \hline 
3C452 & 152.2 & 10.99 & 184.52 &  & 1.21 & VLSS 2 comp.\\ % \tabularnewline
% \hline 
3C390.3 & 99 & 6.002 & 121.89 &  & 1.23  & VLSS 3 comp.\\ % \tabularnewline
% \hline 
3C380 & 156.2 & 2.9981 & 156.2 &  & 1 & Scaife et. al\\ % \tabularnewline
% \hline 
3C295 & 134.5 & 8.3135 & 95.23 &  & 0.71 & Scaife et. al\\ % \tabularnewline
% \hline 
...
\tablefoot{The $S_{60}$  flux has been  derived from the best  available modeled
  spectra,  or -0.7  otherwise.   The  calibrated flux  ratio  is derived  after
  imaging the  calibrated visibilites and  generating light-curves of  peak flux
  from sources in the snapshots.}
\end{table*}
...
compared to  e.g., WSF  (see Table \ref{tab:Performance-results}).  This implies
that their  number can be increased in  order to handle data  with higher errors
with little effect on latency bounds.  Thus, the results from a single iteration
show the worst case calibration (and best case computational) performance of our
approach.
...
\ref{tab:Performance-results}. A schematic view  of the tracking calibration and
its  major  components  is  shown  in  Figure  \ref{fig:trackcalSchematic}.  The
calibration solution memory pool is utlized by the goodness of solution block to
decide on the propagation of an instantaneous solution.
...
data   affected   by    RFI   in   the   Low   Band    is   surprisingly   small
\citep{offringa2012lofar},    and   is    not   considered    to    affect   our
...
curves. Here,  the stars  indicate calibration with  no prior knowledge  of gain
solutions,  as used  in  convergent  calibration, while  the  diamonds show  the
convergence  rate  after  pre-initializing  the  solver with  solutions  from  a
previous timeslice.  We see  that after initializing  antenna gain  estimates to
unity,  the  DIE calibration  converges  in tens  of  iterations,  with a  steep
convergence curve.  This is a motivation  to apply tracking  calibration for the
AARTFAAC.
...
we  plot the  modelling error  as a  function of  skymodel complexity,  which is
...
that  a  low   complexity  model  is  adequate.   The   right  panel  of  Figure
\ref{fig:The-model-amplitude} shows  the phase  of the ratio  between calibrated
and  model visibilities  as a  function  of calibrated  visibility power.   This
should be  close to 0, but has  an error contribution due  to unmodeled sources,
which constitute the real information from the calibrated image. The large phase
scatter at  the low residual power  end is contributed by  visibilities with low
SNR, thus  having a  higher calibration error.  Visibilities with high  SNR show
<0.5 rad phase error, implying that the simple point source sky model containing
a large fraction  of received flux is adequate  for snapshot calibration.This is
typical for  the AARTFAAC, unless  RFI or other  unmodeled sources have  a large
contribution.
...
\caption{AARTFAAC zenith projection  all sky images at ~60  MHz, with 24kHz
...
function of  integration time. The difference  image curve follows  the trend of
the theoretical thermal noise behaviour closely upto about a minute, after which
Earth rotation is expected to  introduce systematic offsets between sources. The
